...
class KAN(nn.Module):
    ...
    def __init__(
        ...
        for l in range(self.depth):
            ...
            scale_base = (
                1 / np.sqrt(width[l])
                + (torch.randn(width[l] * width[l + 1]) * 2 - 1)
                * noise_scale_base
            )
            sp_batch = KANLayer(
                in_dim=width[l],
            ...
            # bias
            bias = nn.Linear(
                width[l + 1], 1, bias=False, device=device
            ).requires_grad_(bias_trainable)
    ...
    def forward(self, x):
        ...
        for l in range(self.depth):
            ...
            x_numerical, preacts, postacts_numerical, postspline = self.act_fun[
                l
            ](x)
    ...
    def plot(
        ...
        skeleton_files = [e for e in os.listdir(folder) if 'skeleton' in e]
        filename = f'{folder}/skeleton_{len(skeleton_files)}.png'
    ...
    def train(
        ...
        def closure():
            ...
            if sglr_avoid == True:
                id_ = torch.where(torch.isnan(torch.sum(pred, dim=1)) == False)[
                    0
                ]
                train_loss = loss_fn(
